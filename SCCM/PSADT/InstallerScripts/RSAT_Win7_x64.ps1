...
Try {
	...
	[string]$appScriptAuthor = 'Jonathon Anderson'
	##*===============================================
	...
	If ($deploymentType -ine 'Uninstall') {
        ...
        [String]$WusaEXE = (Get-Item -Path $env:windir\System32\wusa.exe).FullName
        [String]$WusaArgs = "/quiet /norestart `"$dirFiles\Windows6.1-KB958830-x64-RefreshPkg.msu`""

        [hashtable]$InstallSplat = @{ Path = $WusaEXE; Parameters = $WusaArgs; PassThru = $True }
        $InstallExitCode = Execute-Process @InstallSplat
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		[String]$WusaEXE = (Get-Item -Path $env:windir\System32\wusa.exe).FullName
        [String]$WusaArgs = "/quiet /norestart /uninstall `"$dirFiles\Windows6.1-KB958830-x64-RefreshPkg.msu`""

        [hashtable]$UnInstallSplat = @{ Path = $WusaEXE; Parameters = $WusaArgs; PassThru = $True }
        $UnInstallExitCode = Execute-Process @UnInstallSplat
